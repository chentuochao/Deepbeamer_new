...
class Audio_Set:
    ...
    def __read_audio(self,file, begin, end):
        #print(self.path + file)
        audio, _ = librosa.load(self.path + file, sr=fs, mono=True)
        #print(audio.shape, int(begin*fs), int(end*fs), begin, end )
        return audio[int(begin*fs) : int(end*fs)]
    def __getitem__(self, idx):
        return self.__read_audio(self.audio_files[idx][0], self.audio_files[idx][2], self.audio_files[idx][3]),  self.audio_files[idx][1]
...
class OnlineSimulationDataset(Dataset):
    def __init__(self, voice_collection_pure, noise_collection, length, simulation_config, truncator, cache_folder, cache_max=None):
        self.voices=voice_collection_pure
    ...
    def __getitem__(self, idx):
        ...
        remain_number = n_source
        voices = []
        voice_labels = []
        while remain_number > 0:
            index = np.random.choice(len(self.voices))
            voice_data = self.voices[index]
            v = self.truncator.process(voice_data[0])
            if np.max(np.abs(v)) <= 0:
                print("Invalid audio file, try again")
            else:
                voices.append(v)
                voice_labels.append(voice_data[1])
                remain_number -= 1

        label_options = find_all_label_combination(voice_labels)
