...
def find_all_label_combination(labels):
    exist_index = []
    num_classes = labels[0].shape[0]
    for l in labels:
        for i in range(0, num_classes):
            if l[i] == 1 and i not in exist_index: exist_index.append(i)
    ...
    for i in range(1, 2**num):
        ...
        label_inv = np.ones(num_classes)
        for j in range(0, num):
            if (i >> j) & 1 == 1: 
                label[exist_index[j]] = 1
                label_inv[exist_index[j]] = 0
        mix_index = []
        possible = True 
        
        for j in range(0, len(labels)):
            #print(label, labels[j])  
            if np.sum(np.multiply(label, labels[j])) > 0:
                if np.sum(np.multiply(label_inv, labels[j])) == 0:
                    mix_index.append(j)
                else:
                    possible = False
                    break 
         
        if possible and len(mix_index) > 0 : label_options.append((label, mix_index))
