...
class ConvTasNet(nn.Module):
    def __init__(self, N, L, B, H, P, X, R, C, num_class, norm_type="gLN", causal=False,
                 mask_nonlinear='relu'):
        """
        Args:
        ...
        self.separator = TemporalConvNet(N, B, H, P, X, R, C,num_class, norm_type, causal, mask_nonlinear)
    ...
    def forward(self, mixture, label_vector):
        ...
        est_mask = self.separator(mixture_w, label_vector)
...
class Encoder(nn.Module):
    ...
    def forward(self, mixture):
        ...
        mixture_w = self.layer1(mixture) # [M, N, K]
        return mixture_w
...
class Decoder(nn.Module):
    ...
    def forward(self, mixture_w, est_mask):
        """
        Args:
            mixture_w: [M, N, K] n = 32, k = 512
            est_mask: [M, C, N, K]
        Returns:
            est_source: [M, C, T]
        """
        # D = W * M
        #print(mixture_w.size(), est_mask.size())
...
class TemporalConvNet(nn.Module):
    def __init__(self, N, B, H, P, X, R, C, num_class, norm_type="gLN", causal=False,
                 mask_nonlinear='relu'):
        """
        Args:
            N: Number of filters in autoencoder
        ...
        # [M, N, K] -> [M, N, K]
        #print(N, num_class)
        layer_norm = ChannelwiseLayerNorm(N + num_class)
        # [M, N, K] -> [M, B, K]
        bottleneck_conv1x1 = nn.Conv1d(N + num_class, B, 1, bias=False)
        # [M, B, K] -> [M, B, K]
        repeats = []
        for r in range(R):
            blocks = []
        ...
        if self.mask_nonlinear == 'softmax':
            ...
            self.act = nn.ReLU()
            #est_mask = nn.relu(score)

    def forward(self, mixture_w, label_vector):
        """
        Keep this API same with TasNet
        Args:
            mixture_w: [M, N, K], M is batch size K=512, N = 32
        returns:
            est_mask: [M, C, N, K]
        """
        M, N, K = mixture_w.size()
        label_vector = torch.unsqueeze(label_vector, 2)
        #print(label_vector.size())
        label_vector = label_vector.repeat(1, 1, K)
        #print(label_vector.size(), mixture_w.size())
        #print(mixture_w.type(), label_vector.type())
        features = torch.cat((label_vector, mixture_w), dim=1)
        #print(features.type())
        score = self.network(features)  # [M, N, K] -> [M, C*N, K]
        score = score.view(M, self.C, N, K) # [M, C*N, K] -> [M, C, N, K]
        est_mask = self.act(score)
        '''
        if self.mask_nonlinear == 'softmax':
            est_mask = F.softmax(score, dim=1)
        elif self.mask_nonlinear == 'relu':
...
if __name__ == "__main__":
    ...
    separator = TemporalConvNet(N, B, H, P, X, R, C, Class_num, norm_type=norm_type, causal=causal)
    
    est_mask = separator(mixture_w,temp_labels)
    ...
    # test Conv-TasNet
    conv_tasnet = ConvTasNet(N, L, B, H, P, X, R, C, Class_num, norm_type=norm_type)
